...
contemplate future moves before selecting one from the current state. These search algos operate using models
that have both a policy head and a value head.
"""
...
def null_search(state: str, model, temp: float = 1.0, **kwargs) -> Tuple[int, float, np.ndarray, Tuple[int]]:
    ...
    :return:
        ...
        - state_value (float): The estimated terminal reward [-1, +1] for the current player.
...
def naive_search(state: str, model, batch_size: int = 64, gamma: float = 1.0, **kwargs
    ...
    """
    Performs a naive search where the next action selected is the one which results in the highest estimated
    next state value according to the model. This function performs a depth 1 forward search, so the
    policy_logits outputs of the model are not used for anything i.e. all possible next actions are evaluated.
...
def minimax_search(state: str, model, batch_size: int = 64, gamma: float = 1.0, horizon: int = 3, **kwargs
                   ) -> Tuple[int, float, np.ndarray, Tuple[int]]:
    """
...
def monte_carlo_tree_search(state: str, model, batch_size: int = 32, n_iters: int = 200, **kwargs
    ...
    to perform bootstrapping and also produces policy logits that guide exploration towards more attractive
    pathways.
