...
class SupervisedImitationDataset(Dataset):
    ...
    stockfish best move recommendations from the lichess evals database of over 100mm unique boards.
    """

    def __init__(self, dataset_path: str, state_to_model_input: Callable):
        """
        Initializes a dataset object for supervised training of a policy-value model.

        :param dataset_path: A file path pointing to the supervised imitation learning parquet dataset.
...
def get_dataloader(batch_size: int, dataset_path: str, state_to_model_input: Callable) -> DataLoader:
    """
    Creates a dataloader for supervised imitation learning of a policy-value network.
...
class Trainer:
    ...
    def load(self, milestone: int) -> None:
        ...
        if self.reset_lr_scheduler:  # If True, do not load the prior learning rate scheduler state
            ...
            msg = f"Creating a new learning rate scheduler object, lr_start={self.start}, "
...
def run_training(config_name: str) -> None:
    """
    This function runs supervised imitation learning for a given model config name.

    :param config_name: The name of the config to run supervised imitation learning for.
    :returns: None.
    """
    ...
    trainer = Trainer(model=model, train_dataloader=train_dataloader, val_dataloader=val_dataloader,
                      results_folder=config["output"]["output_path"], **config["training"])
    trainer.train()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the supervised imitation learning loop",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--config", help="The name of the config file to be used for training.")
    args = parser.parse_args()
    run_training(args.config)  # Run supervised imitation learning
